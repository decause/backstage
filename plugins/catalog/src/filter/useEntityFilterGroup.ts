...
export const useEntityFilterGroup = (
  ...
  // The initial selected filters is only considered once on mount
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const initialMemo = useMemo(() => initialSelectedFilters, []);
  ...
  useEffect(() => {
    ...
    groupsContext.register(filterGroupId, filterGroup, initialMemo);
  ...
  const setSelectedFilters = useCallback(
    (filters: string[]) => {
      groupsContext.setGroupSelectedFilters(filterGroupId, filters);
    },
